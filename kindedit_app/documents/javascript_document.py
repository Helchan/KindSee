...
class JavaScriptDocumentType:
    ...
    file_dialog_patterns = ("*.js", "*.mjs", "*.cjs")
    view_mode = "text"
    ...
    def parse(self, text: str) -> ParseResult:
        ...
        if len(text) > MAX_JAVASCRIPT_CHECK_CHARS:
            return ParseResult(None, EmptyPositionIndex(), "", False)
        error = _check_javascript_balance(text)
        if error:
            return ParseResult(None, EmptyPositionIndex(), error, True)
        return ParseResult(None, EmptyPositionIndex(), "", False)

    def format_text(self, text: str) -> str:
        raise ValueError("JavaScript 文件暂不支持格式化")

    def compact_text(self, text: str) -> str:
        raise ValueError("JavaScript 文件暂不支持压缩")
...
def _check_javascript_balance(text: str) -> str:
